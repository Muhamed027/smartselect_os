<div class="max-w-5xl mx-auto mt-8 space-y-5 ">
    ...
    <div class="flex items-center h-full justify-between space-x-2">
        ...
        <div class="w-full">
            <x-forms.input-label for="statuses" :value="__('statuses :')" />
            <select wire:model.live="status" name="status" id="status" required class='select-menu'>
                <option disabled selected> pick a category</option>
                @foreach ($statuses as $status)
                    <option value="{{ $status->id }}" class="select-option">{{ $status->name }}</option>
                @endforeach
            </select>
    ...
    <div>
        ...
        <x-forms.input-textarea wireState="body" areaHeight="4" value="{{ old('body') }}" />
